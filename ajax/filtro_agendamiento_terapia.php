...
$action='ajax';
	if($action == 'ajax'){
         $q = mysqli_real_escape_string($con,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
		  $sTable = "agen_terapia";
		 $sWhere = "";
		 $sWhere.="WHERE id_agen_terapia<=1000";
		if ( $_GET['q'] != "" )
		{
		...
		$reload = './mostrar_agendamiento_terapia.php';
		//main query to fetch the data
		$sql="SELECT alumnos.id_alumno,alumnos.run_alumno,alumnos.nom_alumno,fecha_terapia,hora_terapia          
        FROM  $sTable 
      
        inner join alumnos on(ingreso_terapia.id_alumno=alumnos.id_alumno)
		...
		if ($numrows>0){
			...
			<table class="table table-hover" style="border-color: #A6DAEA;">
				...
				while ($row=mysqli_fetch_array($query)){
						$fecha=$row['fecha_terapia'];
						$hora=$row['hora_terapia'];
