...
$action='ajax';
	if($action == 'ajax'){
		...
		$sql="SELECT id_agen_masoterapia,run_alumno,nom_alumno,edad,fecha_nac,telefono,nombre_apoderado,telefono_apod,direccion_alumno,comuna,correo,patologias,
		...
		if ($numrows>0){
			...
			<table class="table table-hover" style="border-color: #A6DAEA;width:2500px;height:100px ">
				...
				while ($row=mysqli_fetch_array($query)){
					...
					?>
					<tr>
					<td width="10px"> <?php echo ($id);echo"<input type='hidden'  name='id_agen_maso[]' value='".$id."'>";?></td>
