...
$action='ajax';
	if($action == 'ajax'){
		...
		$sql="SELECT id_agen_psico,run_alumno,nom_alumno,edad,fecha_nac,telefono,nombre_apoderado,telefono_apod,direccion_alumno,comuna,correo,patologias,
        talleres.nom_taller,talleres.id_taller,fecha_psico,hora_psico,diagnostico,sesion,salas.id_sala,salas.nom_sala,profesionales.nom_profesional,profesionales.id_profesional  
        FROM  $sTable 
		...
		if ($numrows>0){
			...
			<table class="table table-hover" style="border-color: #A6DAEA;width:2500px;height:100px ">
				...
				while ($row=mysqli_fetch_array($query)){
					...
					?>
<tr>
					<td width="10px"> <?php echo ($id);echo"<input type='hidden'  name='id_agen_psico[]' value='".$id."'>";?></td>
			...
			echo '       <th  style="width:400px" scope="col">Direccion</th>';
			echo '       <th  style="width:400px" scope="col">Comuna</th>';
			echo '       <th  style="width:400px" scope="col">Nom.Apod.</th>';
			echo '       <th  style="width:400px" scope="col">Fono.Apod</th>';
