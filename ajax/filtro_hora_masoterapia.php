...
$action='ajax';
	if($action == 'ajax'){
         $q = mysqli_real_escape_string($con,(strip_tags($_REQUEST['q'], ENT_QUOTES)));
		  $sTable = "ingreso_masoterapia";
		 $sWhere = "";
		 $sWhere.="WHERE id_hora_masoterapia<=1000";
		if ( $_GET['q'] != "" )
		{
		...
		$reload = './buscar_hora_masoterapia.php';
		//main query to fetch the data
		$sql="SELECT alumnos.id_alumno,alumnos.run_alumno,alumnos.nom_alumno
        ,fecha_ingreso,hora_ingreso    
        FROM  $sTable 
        inner join alumnos on(ingreso_masoterapia.id_alumno=alumnos.id_alumno)
