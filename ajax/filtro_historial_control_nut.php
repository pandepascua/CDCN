...
$action='ajax';
	if($action == 'ajax'){
		...
		$sql="SELECT id_agen_nutricion,run_alumno,nom_alumno,edad,telefono,correo,
        talleres.nom_taller,talleres.id_taller,tipo_asistencia.id_tipo_asis,tipo_asistencia.tipo_asis,fecha_nutricion,hora_nutricion,motivo_consulta_nut,diagnostico_nut,observacion_nut,sesion,salas.id_sala,salas.nom_sala,profesionales.nom_profesional,profesionales.id_profesional  
        FROM  $sTable 
		inner join tipo_asistencia on(asis_control_nut_ing.id_tipo_asis=tipo_asistencia.id_tipo_asis)

        inner join talleres on(asis_control_nut_ing.id_taller=talleres.id_taller)
        inner join salas on(asis_control_nut_ing.id_sala=salas.id_sala)
        inner join profesionales on(asis_control_nut_ing.id_profesional=profesionales.id_profesional) 
